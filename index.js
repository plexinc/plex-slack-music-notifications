  ...
  if (payload.Player.uuid = process.env.PLAYER && payload.Account.id === 1 && payload.Metadata.type === 'track') {
    ...
    if (payload.event == "media.play" || payload.event == "media.resume") {
      request.post({
        ...
        body:    `${common}&profile=%7B%22status_text%22%3A%22${encodeURIComponent(payload.Metadata.grandparentTitle)}%20-%20${encodeURIComponent(payload.Metadata.title)}%22%2C%22status_emoji%22%3A%22%3A${process.env.PLAY_EMOJI}%3A%22%7D`
      }, (error, response, body) => {});
